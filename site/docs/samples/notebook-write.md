...
This sample shows three scenarios (a) how fybrik prevents writing a new asset due to governance restrictions (b) how to write data generated by the workload to an object store. (c) how to read data from a dataset stored in an object store
...
Define an [OpenPolicyAgent](https://www.openpolicyagent.org/) policy to forbid to write sensitive data to regions `neverland` and `theshire` in datasets tagged with `finance`. This policy prevents the writing as the deployed fybrik storage account resources applied are in `neverland` and `theshire`.
