...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: datauserclient
        image: image-registry.openshift-image-registry.svc:5000/"$KUBE_NAMESPACE"/datauserclient:latest
        imagePullPolicy: Always
        ports:
        - containerPort: 3000
      - name: datauserserver
        image: image-registry.openshift-image-registry.svc:5000/"$KUBE_NAMESPACE"/datauserserver:latest
        imagePullPolicy: Always
        envFrom:
        - configMapRef:
            name: m4d-config
