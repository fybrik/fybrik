...
//     Then an asset is created with name: <ResourceMetadata.Name>-<Kubernetes Generated Random String>
...
func (r *Handler) createAssetInfo(c *gin.Context) {
	...
	var err error
	assetNamePrefix := FybrikAssetPrefix
	if request.ResourceMetadata.Name != "" {
		assetNamePrefix = request.ResourceMetadata.Name + "-"
	}

	asset = &v1alpha1.Asset{
		ObjectMeta: v1.ObjectMeta{Namespace: request.DestinationCatalogID, Name: request.DestinationAssetID, GenerateName: assetNamePrefix},
	...
	if request.DestinationAssetID != "" {
		// scenario when asset name is not generated by kubernetes
		// update the name field in the resourcemetadata to be asset name for consistency
		// this step ensures we dont call controller runtime update method later and hence we can save a method call.
		asset.Spec.Metadata.Name = request.DestinationAssetID
	}
	...
	if err != nil {
		r.reportError(c, http.StatusInternalServerError, "Error during create asset. Error:"+err.Error())
		return
	}
	logging.LogStructure("Created Asset: ", asset, &r.log, false, false)

	// scenario when asset name is generated by kubernetes with appropriate assetPrefix
	// update the name field in the resourcemetadata by calling controller runtime update method to be asset name for consistency
	if request.DestinationAssetID == "" {
		...
		if err != nil {
			r.reportError(c, http.StatusInternalServerError, "Update asset failed after retrying for 5 seconds. Error:"+err.Error())
			return
		}
