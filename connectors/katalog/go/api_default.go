...
func GetAssetInfoPost(c *gin.Context) {
	input := new(datacatalog.GetAssetRequest)
	...
	var columns []datacatalog.ResourceColumn
	if len(asset.Spec.AssetMetadata.Columns) > 0 {
		columns = make([]datacatalog.ResourceColumn, len(asset.Spec.AssetMetadata.Columns))
		for i, c := range asset.Spec.AssetMetadata.Columns {
			columns[i] = datacatalog.ResourceColumn{
				Name: c.Name,
				Tags: c.Tags,
			}
		}
	...
	output := datacatalog.GetAssetResponse{
