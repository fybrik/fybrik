...
// The function returns an error received whilw communication with connectors or evaluating policies
// It also returns connector messages to be propagated to the application status (relevant for the ready state of the asset)
func (r *FybrikApplicationReconciler) constructDataInfo(req *datapath.DataInfo, appContext ApplicationContext,
	...
	if !req.Context.Requirements.FlowParams.IsNewDataSet {
		...
		if response, err = r.DataCatalog.GetAssetInfo(&request, credentialPath); err != nil {
			...
			// return the error (from the catalog connector)
			return "", err
		}
	...
	if err != nil {
		// return the error received from the connector, or generated by Fybrik in case of Deny
	...
	if err != nil {
		...
		// return the error (from config policy evaluator)
		return "", err
	}
	...
	// data has been collected - return messages from the catalog and policy manager
