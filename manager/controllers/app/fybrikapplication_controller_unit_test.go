...
// FybrikStorageAccount has unstructured data thus its own decode method is applied
func readStorageAccountData(f string, obj *fappv2.FybrikStorageAccount) error {
	bytes, err := os.ReadFile(f)
	if err != nil {
		return err
	}
	return obj.DecodeYaml(bytes)
}
...
func TestFybrikApplicationControllerCSVCopyAndRead(t *testing.T) {
	...
	g.Expect(copyFlow.Triggers).To(gomega.ContainElements(fappv1.InitTrigger))
	readFlow := flow.SubFlows[1]
	g.Expect(readFlow.FlowType).To(gomega.Equal(taxonomy.ReadFlow))
	g.Expect(readFlow.Triggers).To(gomega.ContainElements(fappv1.WorkloadTrigger))
...
func TestDenyOnRead(t *testing.T) {
	...
	application := &fappv1.FybrikApplication{}
	g.Expect(readObjectFromFile("../../testdata/unittests/data-usage.yaml", application)).NotTo(gomega.HaveOccurred())
	application.Spec.Data[0] = fappv1.DataContext{
		DataSetID:    "s3/deny-dataset",
		Requirements: fappv1.DataRequirements{Interface: &taxonomy.Interface{Protocol: mockup.S3, DataFormat: mockup.Parquet}},
	}
	application.SetGeneration(1)
...
func TestNoReadPath(t *testing.T) {
	...
	application.Spec.Data[0] = fappv1.DataContext{
		...
		Requirements: fappv1.DataRequirements{Interface: &taxonomy.Interface{Protocol: mockup.JdbcDB2}},
	}
	application.SetGeneration(1)
...
func TestMultipleDatasets(t *testing.T) {
	...
	g.Expect(application.Status.Generated.Namespace).To(gomega.Equal(environment.GetSystemNamespace()))
	plotterObjectKey := types.NamespacedName{
...
// This test checks the case where data comes from another geography, and should be redacted.
...
func TestCopyData(t *testing.T) {
	...
	g.Expect(subflow.Triggers).To(gomega.ContainElements(fappv1.InitTrigger))
...
// A storage account has been defined for the geography where the dataset can not be written to according to governance policies.
...
func TestStorageCost(t *testing.T) {
	...
	g.Expect(cl.Create(context.TODO(), dummySecret)).NotTo(gomega.HaveOccurred())
	account := &fappv2.FybrikStorageAccount{}
	g.Expect(readStorageAccountData("../../testdata/unittests/account-neverland.yaml", account)).NotTo(gomega.HaveOccurred())
	account.Namespace = adminNamespace
	g.Expect(cl.Create(context.TODO(), account)).NotTo(gomega.HaveOccurred())
...
func TestSyncWithPlotter(t *testing.T) {
	...
	systemNamespace := environment.GetSystemNamespace()
	application := &fappv1.FybrikApplication{}
	...
	plotter.Status.ObservedState.Ready = true
	plotter.Namespace = systemNamespace
	...
	newApp := &fappv1.FybrikApplication{}
...
func TestFybrikApplicationWithInvalidAppInfo(t *testing.T) {
	...
	fybrikApp := &fappv1.FybrikApplication{}
...
func TestCopyModule(t *testing.T) {
	...
	// Create a fake client to mock API calls.
	cl := fake.NewFakeClientWithScheme(s, objs...)
	invalidModule := &fappv1.FybrikModule{}
	...
	invalidModule.Namespace = adminNamespace
	...
	capability.SupportedInterfaces = []fappv1.ModuleInOut{{Source: source}, {Sink: sink}}
	...
	secret.Namespace = adminNamespace
...
func TestEmptyInterface(t *testing.T) {
	...
	module.Namespace = environment.GetAdminNamespace()
