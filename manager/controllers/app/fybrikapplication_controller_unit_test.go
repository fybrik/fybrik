...
func TestFybrikApplicationControllerCSVCopyAndRead(t *testing.T) {
	...
	application := &v12.FybrikApplication{}
	...
	g.Expect(copyFlow.Triggers).To(gomega.ContainElements(v12.InitTrigger))
	readFlow := flow.SubFlows[1]
	g.Expect(readFlow.FlowType).To(gomega.Equal(taxonomy.ReadFlow))
	g.Expect(readFlow.Triggers).To(gomega.ContainElements(v12.WorkloadTrigger))
...
func TestDenyOnRead(t *testing.T) {
	...
	application := &v12.FybrikApplication{}
	g.Expect(readObjectFromFile("../../testdata/unittests/data-usage.yaml", application)).NotTo(gomega.HaveOccurred())
	application.Spec.Data[0] = v12.DataContext{
		DataSetID:    "s3/deny-dataset",
		Requirements: v12.DataRequirements{Interface: &taxonomy.Interface{Protocol: v12.S3, DataFormat: v12.Parquet}},
	}
	application.SetGeneration(1)
	...
	g.Expect(cond.Message).To(gomega.ContainSubstring(v12.ReadAccessDenied))
...
func TestNoReadPath(t *testing.T) {
	...
	application.Spec.Data[0] = v12.DataContext{
		...
		Requirements: v12.DataRequirements{Interface: &taxonomy.Interface{Protocol: v12.JdbcDB2}},
	}
	application.SetGeneration(1)
...
func TestWrongCopyModule(t *testing.T) {
	...
	application := &v12.FybrikApplication{}
	g.Expect(readObjectFromFile("../../testdata/unittests/data-usage.yaml", application)).NotTo(gomega.HaveOccurred())
	application.Spec.Data = []v12.DataContext{
		{
			DataSetID:    "s3/allow-dataset",
			Requirements: v12.DataRequirements{Interface: &taxonomy.Interface{Protocol: v12.ArrowFlight}},
		},
		{
...
func TestCopyData(t *testing.T) {
	...
	g.Expect(subflow.Triggers).To(gomega.ContainElements(v12.InitTrigger))
...
func TestCopyDataNotAllowed(t *testing.T) {
	...
	g.Expect(cond.Status).To(gomega.BeIdenticalTo(corev1.ConditionTrue), "Deny condition is not set")
	g.Expect(cond.Message).To(gomega.ContainSubstring(v12.WriteNotAllowed))
	g.Expect(application.Status.Ready).To(gomega.BeTrue())
}
...
func TestSyncWithPlotter(t *testing.T) {
	...
	controllerNamespace := environment.GetControllerNamespace()
	fmt.Printf("FybrikApplication unit test: controller namespace " + controllerNamespace)
	application.Status.Generated = &v12.ResourceReference{Name: "plotter", Namespace: controllerNamespace, Kind: "Plotter", AppVersion: 1}
...
func TestFybrikApplicationWithInvalidAppInfo(t *testing.T) {
	...
	fybrikApp := &v12.FybrikApplication{}
...
func TestCopyModule(t *testing.T) {
	...
	// Create a fake client to mock API calls.
	cl := fake.NewFakeClientWithScheme(s, objs...)
	invalidModule := &v12.FybrikModule{}
	...
	capability.SupportedInterfaces = []v12.ModuleInOut{{Source: source}, {Sink: sink}}
...
func TestWriteUnregisteredAsset(t *testing.T) {
	...
	// Read module
	readWriteModule := &v12.FybrikModule{}
	...
	g.Expect(err).NotTo(gomega.HaveOccurred())
	g.Expect(plotter.Spec.Assets).To(gomega.HaveLen(1))
	g.Expect(plotter.Spec.Assets["s3-not-exists/new-dataset"].DataStore.Connection.Name).To(gomega.Equal(v12.S3))
