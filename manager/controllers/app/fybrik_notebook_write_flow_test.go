...
// Deny-by-default policy will prevent the writing.
func testWriteDenyByDefault(t *testing.T, k8sClient client.Client) {
	...
	writeApplication.Name += "-1"
	writeApplicationKey := client.ObjectKeyFromObject(writeApplication)
	writeApplication.Spec.AppInfo.Properties.Items["type"] = "unknown"
...
// Governance policy will prevent the writing.
func testWriteDeny(t *testing.T, k8sClient client.Client) {
	fmt.Println("Starting deny write by explicit policy")
	var err error
	g := gomega.NewWithT(t)
	// Installing application
	writeApplication := &fappv1.FybrikApplication{}
	g.Expect(readObjectFromFile("../../testdata/notebook/write-flow/fybrikapplication-write.yaml", writeApplication)).
		ToNot(gomega.HaveOccurred())
	writeApplication.Name += "-2"
	writeApplicationKey := client.ObjectKeyFromObject(writeApplication)
	writeApplication.Spec.AppInfo.Properties.Items["type"] = "test"
	// Create FybrikApplication
	g.Expect(k8sClient.Create(context.Background(), writeApplication)).Should(gomega.Succeed())
...
func testWriteAllowed(t *testing.T, k8sClient client.Client) {
	// This test checks the following scenarios:
	// (a) how to write data generated by the workload to an object store.
	// (b) how to read data from a dataset stored in an object store
	...
	writeApplicationKey := client.ObjectKeyFromObject(writeApplication)
	writeApplication.Spec.AppInfo.Properties.Items["type"] = "prod"
	// Create FybrikApplication
...
func TestS3NotebookWriteFlow(t *testing.T) {
	...
	_ = fappv1.AddToScheme(scheme.Scheme)
	_ = fappv2.AddToScheme(scheme.Scheme)
	k8sClient, err := client.New(ctrl.GetConfigOrDie(), client.Options{Scheme: scheme.Scheme}) //nolint:govet
	g.Expect(err).To(gomega.BeNil())
	testWriteDenyByDefault(t, k8sClient)
	testWriteDeny(t, k8sClient)
	testWriteAllowed(t, k8sClient)
}
