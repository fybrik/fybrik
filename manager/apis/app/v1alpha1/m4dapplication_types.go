...
type ApplicationDetails struct {
	...
	// Name of the person doing the processing
	// +optional
	AppUser string `json:"appUser"`
}

// RecurrenceType defines how often the data load should be repeated
// +kubebuilder:validation:Enum=none;daily;weekly;monthly
...
// WriteDataContext indicates uncataloged data set chosen by the Data Owner to be ingested,
// or a new data set that is being created which may need to be cataloged.
// and includes information about the data format and technologies used by the application
// to access the data, and the residency of the data - i.e. the geography in which it was created.
type WriteDataContext struct {
	// DataSetID is a unique identifier of the dataset in the data catalog.
	// None for new data being generated by the application, nor for a new data set being ingested for the first time.
	// If exists, append data to the existing asset.  Get destination details from catalog.
	// +optional
	DataSetID string `json:"dataSetID"`

	// Source data store to be copied into the M4D managed environment if existing data is being ingested
	// Required if ingesting existing uncataloged data.
	// +optional
	...
	// Recurrence indicates if the ingest of external data should be repeated more than once
	// +optional
	Recurrence RecurrenceType `json:"recurrence"`

	// IFdetails indicates the protocol and format for accessing the data
	// Required if ingesting existing uncataloged data.
	// +optional
	IFdetails InterfaceDetails `json:"ifDetails"`
}
...
type M4DApplicationSpec struct {
	...
	// Note: either Data or WriteData must have at least one entry
	// +optional
	Data []DataContext `json:"data"`

	// WriteData is data that is not yet in the data catalog and thus not governed.
	// It may be data from an external system that is being ingested, or it may be new data
	// being generated by a user's application.
	// The params in the structure indicate how to access the source data in order to load the data
	// into a data store in the controlled environment and to catalog it in the data catalog.
	// Note: either Data or WriteData must have at least one entry
	// +optional
	WriteData []WriteDataContext `json:"writeData"`
}
